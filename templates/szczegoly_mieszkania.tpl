...
<table style="width: 100%; min-height: 200px">
    <tr>
        <td style="border: 1px solid black; text-align: left; vertical-align: top">
            ...
            Nr lokalu: {$oferta.nr_lokalu}<br />
            Cena: {$oferta.cena}<br />
            Liczba pokoi: {$oferta.pokoje}<br />
            Powierzchnia mieszkania: {$oferta.pow_mieszkania}<br />
            Rok budowy: {$oferta.rok}<br />
            Piętro: {$oferta.pietro}<br />
            Liczba pięter: {$oferta.liczba_pieter}<br />
            Winda: {if $oferta.winda=='t'}jest{else}nie ma{/if}<br />
            Materiał: {$oferta.material}<br />
            Stan lokalu: {$oferta.stan_lokalu}<br />
            Stan budynku: {$oferta.stan_budynku}<br />
            Garaż: {if $oferta.garaz=='t'}jest{else}nie ma{/if}<br />
            Osiedle: {$oferta.osiedle}<br />
...
<table style="width: 100%">
    <tr>
        ...
        <td style="border: 1px solid black">{if $koszyk}<a href="koszyk.php">Wróć do koszyka >></a>{else}<a href="wyniki_mieszkania.php">Wróć do wyników >></a>{/if}</td>
    </tr>
</table>
<br />
